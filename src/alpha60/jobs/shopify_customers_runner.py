"""Composition root for Shopify customer ingestion."""
...
from alpha60.jobs.shopify_customers import load_shopify_customers
...
_SHOPIFY_CUSTOMERS_JOB_NAME = "shopify-customers"
_SHOPIFY_CUSTOMERS_CURSOR_FIELD = "updated_at"


def run_shopify_customers_ingestion(
    settings: Settings,
    max_pages: int | None = None,
) -> WarehouseLoadResult:
    """Run the Shopify customers ingestion job using runtime settings."""
    ...
    state = state_repository.get_state(_SHOPIFY_CUSTOMERS_JOB_NAME)

    job_result = load_shopify_customers(
    ...
    if (
        ...
        state_repository.save_state(
            IncrementalState(
                job_name=_SHOPIFY_CUSTOMERS_JOB_NAME,
                cursor_field=_SHOPIFY_CUSTOMERS_CURSOR_FIELD,
