"""Shopify inventory levels staging transformation factory."""
...
_SQL_PATH = Path(__file__).parent / "sql" / "shopify" / "stg_shopify_inventory_levels.sql"


def create_shopify_inventory_levels_staging_transformation(
    client: QueryClient,
    raw_config: BigQueryConfig,
    staging_dataset_id: str,
) -> SqlTransformationRunner:
    """Create the Shopify inventory levels staging transformation."""
    target_table_id = (
        f"{raw_config.project_id}.{staging_dataset_id}.shopify_inventory_levels"
    )
