"""Shopify order lines staging transformation factory."""
...
_SQL_PATH = Path(__file__).parent / "sql" / "shopify" / "stg_shopify_order_lines.sql"


def create_shopify_order_lines_staging_transformation(
    client: QueryClient,
    raw_config: BigQueryConfig,
    staging_dataset_id: str,
) -> SqlTransformationRunner:
    """Create the Shopify order lines staging transformation."""
    target_table_id = (
        f"{raw_config.project_id}.{staging_dataset_id}.shopify_order_lines"
    )
