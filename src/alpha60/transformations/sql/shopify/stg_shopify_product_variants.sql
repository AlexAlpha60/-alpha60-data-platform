...
unnested_variants AS (
  SELECT
    ...
    SAFE_CAST(
      JSON_VALUE(variant, '$.compare_at_price')
      AS NUMERIC
    ) AS compare_at_price,

    SAFE_CAST(
      JSON_VALUE(variant, '$.inventory_quantity')
      AS INT64
    ) AS inventory_quantity,

    SAFE_CAST(
      JSON_VALUE(variant, '$.inventory_item_id')
      AS INT64
    ...
    SAFE_CAST(
      JSON_VALUE(variant, '$.created_at')
      AS TIMESTAMP
    ) AS variant_created_at,

    SAFE_CAST(
      JSON_VALUE(variant, '$.updated_at')
      AS TIMESTAMP
    ) AS variant_updated_at,

    JSON_VALUE(
      TO_JSON(raw_products.payload),
      '$.title'
    ) AS product_title,
  ...
  FROM latest_products AS raw_products
