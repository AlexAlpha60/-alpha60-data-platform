...
SELECT
  ...
  SAFE_CAST(JSON_VALUE(payload, '$.legacy') AS BOOL) AS is_legacy,

  SAFE_CAST(JSON_VALUE(payload, '$.created_at') AS TIMESTAMP) AS location_created_at,
  SAFE_CAST(JSON_VALUE(payload, '$.updated_at') AS TIMESTAMP) AS location_updated_at,
