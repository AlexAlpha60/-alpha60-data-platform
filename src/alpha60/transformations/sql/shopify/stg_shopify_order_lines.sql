...
unnested_order_lines AS (
  SELECT
    ...
    SAFE_CAST(
      JSON_VALUE(line_item, '$.variant_id')
      AS INT64
    ) AS variant_id,
    ...
    CASE
      WHEN JSON_VALUE(line_item, '$.product_id') IS NOT NULL
        THEN 'product'
      WHEN JSON_VALUE(line_item, '$.sku') IS NOT NULL
        AND JSON_VALUE(line_item, '$.sku') != ''
        THEN 'sku_only'
    ...
    SAFE_CAST(
      JSON_VALUE(line_item, '$.quantity')
      AS INT64
    ) AS quantity,

    SAFE_CAST(
      JSON_VALUE(line_item, '$.price')
      AS NUMERIC
    ) AS unit_price,

    SAFE_CAST(
      JSON_VALUE(line_item, '$.total_discount')
      AS NUMERIC
    ) AS total_discount,
  ...
  FROM latest_orders AS raw_orders
