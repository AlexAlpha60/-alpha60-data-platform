"""Tests for Shopify order lines staging transformation composition."""
...
def test_run_shopify_order_lines_staging_transformation_wires_dependencies() -> None:
    ...
    with (
        patch(
            "alpha60.transformations.shopify_order_lines_runner.create_bigquery_client"
        ) as client_factory,
        patch(
            "alpha60.transformations.shopify_order_lines_runner."
        ...
        result = run_shopify_order_lines_staging_transformation(settings=_settings())
...
def test_run_shopify_order_lines_staging_transformation_uses_custom_dataset() -> None:
    ...
    with (
        ...
        result = run_shopify_order_lines_staging_transformation(
            settings=_settings(),
            staging_dataset_id="custom_stg",
        )

    assert result == expected_result
    assert transformation_factory.call_args.kwargs["staging_dataset_id"] == "custom_stg"
