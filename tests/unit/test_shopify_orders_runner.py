"""Tests for Shopify order ingestion composition."""
...
def test_run_shopify_orders_ingestion_wires_dependencies() -> None:
    ...
    with (
        patch(
            "alpha60.jobs.shopify_orders_runner.ShopifyAuthenticator"
        ) as authenticator_class,
        patch("alpha60.jobs.shopify_orders_runner.ShopifyClient") as shopify_client_class,
        ...
        result = run_shopify_orders_ingestion(settings=_settings())
    ...
    state_repository.get_state.assert_called_once_with("shopify-orders")
...
def test_run_shopify_orders_ingestion_passes_existing_cursor() -> None:
    """The runner passes an existing cursor to the Shopify orders job."""
    previous_cursor = datetime(2026, 7, 6, 12, 30, tzinfo=UTC)
    warehouse_result = WarehouseLoadResult(
        table_id="shopify_orders",
        status=WarehouseLoadStatus.SUCCESS,
        rows_loaded=0,
...
def test_run_shopify_orders_ingestion_saves_cursor_after_successful_load() -> None:
    """The runner saves the latest cursor after a successful load."""
    latest_cursor = datetime(2026, 7, 6, 13, 30, tzinfo=UTC)
    warehouse_result = WarehouseLoadResult(
        table_id="shopify_orders",
        status=WarehouseLoadStatus.SUCCESS,
        rows_loaded=1,
    ...
    with (
        ...
        patch("alpha60.jobs.shopify_orders_runner.ShopifyClient"),
        patch("alpha60.jobs.shopify_orders_runner.create_bigquery_loader"),
        patch(
            "alpha60.jobs.shopify_orders_runner.create_bigquery_state_repository"
        ) as state_repository_factory,
        patch("alpha60.jobs.shopify_orders_runner.load_shopify_orders") as load_job,
...
def test_run_shopify_orders_ingestion_does_not_save_cursor_after_failed_load() -> None:
